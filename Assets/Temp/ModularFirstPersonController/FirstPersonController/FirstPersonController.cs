    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            ...
            if (JoystickController.Instance != null &&
                JoystickController.Instance.HasGamepad())
            {
            ...
            // Decide qual entrada usar
            float lookX = Mathf.Abs(rightStick.x) > 0.1f ? rightStick.x : mouseX;
            float lookY = Mathf.Abs(rightStick.y) > 0.1f ? rightStick.y : mouseY;
